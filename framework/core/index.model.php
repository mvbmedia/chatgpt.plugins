...
class Index
{
	...
	public function top_offers()
	{
        ...
        # return top offers
        $this->database->query('SELECT *, (SELECT (AVG(`ratings`.`rating`) * 10) FROM `ratings` WHERE `ratings`.`subject` = `subjects`.`ID`) as `score` FROM `subjects` WHERE `subjects`.`website` = :website AND `status` = :status ORDER BY `rating` DESC, `name` ASC LIMIT 0,5');
	...
	{
		$this->database->query('SELECT * FROM `guide` WHERE `guide`.`website` = :website AND `guide`.`status` = :status ORDER BY `guide`.`ID` DESC LIMIT 0,8');
		$this->database->bind(':website', WEBSITE);
		$this->database->bind(':status', 'active');
		$result = $this->database->resultset();
