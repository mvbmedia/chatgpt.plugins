...
class Functions
{
   ...
   public function tooltip()
   {
       ...
       $this->database->query('SELECT `prompts_count`.`subject`, `prompts_count`.`count`, COUNT(*) as `count` FROM `prompts_count` LEFT JOIN `subjects` ON `prompts_count`.`subject` = `subjects`.`ID` WHERE `subjects`.`ID` = :subject ORDER BY `subjects`.`count');
       $this->database->bind(':subject', $subjects);
       $result = $this->database->resultset();       
